	...
	internal sealed class FixturesLoader
    {
        ...
        private void PlaceCityPieces()
        {
            ...
            foreach (var fixtureRow in this.fixtureRows)
            {
                ...
                nifRow.Polygons = nifRow.Polygons.Select(p => p with { Vectors = new[] { TurnAround(p.P1, center), TurnAround(p.P2, center), TurnAround(p.P3, center) } }).ToArray();
                ...
                try
                {
                    ...
                    else if (fixture.IsTreeCluster)
                    {
                        ...
                        foreach (var tree in fixture.TreeCluster.TreeInstances)
                        {
                            ...
                            {
                                var newPolygon = treePolygon with { Vectors = new[] { treePolygon.P1, treePolygon.P2, treePolygon.P3 } };
